/*!
 * Biblioteca jQuery JavaScript v3.6.3
 * https://jquery.com/
 *
 * Incluye Sizzle.js
 * https://sizzlejs.com/
...
(función(global, fábrica) {
	...
	if ( tipo de módulo === "objeto" && tipo de módulo.exportaciones === "objeto" ) {
		...
		// Ver ticket trac-14549 para más información.
...
var esFuncion = function esFuncion( obj ) {
		...
		// (p. ej., `tipo de documento.getElementsByTagName("div") === "función"`). (gh-4756)
...
var Chisporrotear =
/*!
 * Chisporrotea CSS Selector Engine v2.3.9
 * https://sizzlejs.com/
 *
		...
		si (documentoEsHTML) {
			...
			si (soporte.qsa &&
				...
				intentar {
					...
					// `qSA` no puede arrojar partes no reconocidas utilizando un análisis indulgente:
					// https://drafts.csswg.org/selectors/#forgiving-selector
					// como la pseudo-clase `:has()`:
					// https://drafts.csswg.org/selectors/#relational
					// Todavía se espera que `CSS.supports` devuelva `falso` entonces:
					// https://drafts.csswg.org/css-conditional-4/#typedef-supports-selector-fn
					// https://drafts.csswg.org/css-conditional-4/#dfn-support-selector
					si ( soporte.cssSupportsSelector &&

						// eslint-disable-next-line no-undef
						!CSS.supports( "selector(:es(" + nuevoSelector + "))" ) ) {

						// Soporte: Internet Explorer 11+
						// Lanzar para llegar a la misma ruta de código que un error directamente en qSA.
						// Nota: una vez que solo admitimos el soporte del navegador
						// `CSS.supports('selector(...)')`, lo más probable es que podamos eliminar
						// el `intentar-atrapar`. IE no implementa la API.
...
isXML = Chisporroteo.isXML = función (elemento) {
	var espacio de nombres = elem && elem.namespaceURI,
		docElem = elemento && ( elemento.documentopropietario || elemento ).documentElement;
...
setDocument = Sizzle.setDocument = función (nodo) {
	...
	// Asegúrese de que el modo indulgente no se use en `CSS.supports( "selector(...)" )`.
	//
	// `:is()` usa una lista de selector indulgente como argumento y es ampliamente
	// implementado, por lo que es bueno probarlo.
	soporte.cssSupportsSelector = afirmar ( función () {
		/* eslint-disable no-undef */

		devuelve CSS.supports( "selector(*)" ) &&

			// Soporte: solo Firefox 78-81
			// En el antiguo Firefox, `:is()` no usaba el análisis indulgente. En ese caso,
			// falla esta prueba ya que no hay selector para probar contra eso.
			// `CSS.supports` usa un análisis implacable
			documento.querySelectorAll( ":es(:jqfake)" ) &&

			// Se necesita `*` ya que Safari y Chrome más nuevo implementaron algo intermedio
			// para `:has()` - arroja `qSA` si solo contiene un no soportado
			// argumento, pero varios, uno de los cuales es compatible, están bien.
			// Queremos ir a lo seguro en caso de que `:is()` reciba el mismo tratamiento.
	...
	if ( !support.cssSupportsSelector ) {
		...
		// Soporte: Chrome 105+, Safari 15.4+
		// `:has()` usa una lista de selectores indulgentes como argumento, por lo que nuestro
		// El mecanismo `try-catch` falla al capturar `:has()` con argumentos no admitidos
		// de forma nativa como `:has(:contains("Foo"))`. Cuando sea compatible y cumpla con las especificaciones,
		// ahora usamos `CSS.supports("selector(:is(SELECTOR_TO_BE_TESTED))")`, pero
		// fuera de eso marcamos `:has` como buggy.
	...
	contiene = tiene Comparar || rnative.test( docElem.contains ) ?
		función( a, b ) {
			...
			// IE no tiene `contiene` en `documento`, por lo que debemos verificar
			// Presencia de `documentElement`.
			// Necesitamos recurrir a `a` cuando falta `documentElement`
			// como `propietarioDocumento` de elementos dentro de `<plantilla/>` puede tener
			// uno nulo: un comportamiento predeterminado de todos los navegadores modernos.
	...
	mientras ( hasta ahora ) {
		...
		if ( !emparejado || (coincidencia = rcomma.exec( hasta ahora ) ) ) {
			si (coincidencia) {
				...
				hasta ahora = hasta ahora.slice( partido[ 0 ].longitud ) || tan lejos;
			}
			grupos.push(( tokens = [] ) );
		}
...
function addCombinator(comparador, combinador, base) {
	...
	devolver combinador.primero?
		...
		// Comprobar con todos los antecesores/elementos anteriores
		función (elemento, contexto, xml) {
...
jQuery.extender( {
						...
						función de retorno() {
							var eso = esto,
								...
								podríaLanzar = función() {
									...
									// https://promisesaplus.com/#punto-59
							...
							// https://promisesaplus.com/#punto-57
	...
	cuando: función (valor único) {
		...
		si ( restante <= 1 ) {
			adoptValue( singleValue, primario.hecho( updateFunc( i ) ).resolve, primario.rechazar,
				!restante );
			...
			if (primario.estado() === "pendiente" ||
				...
				volver primario.entonces();
			}
		}

		// Múltiples argumentos se agregan como elementos de la matriz Promise.all
		mientras yo-- ) {
			adoptValue( resolveValues[ i ], updateFunc( i ), primario.rechazar );
		}

		return primario.promesa();
	}
});
...
jQuery.extender( {
	...
	// se dispara el evento ready. Ver trac-6781
	ListoEspera: 1,
...
// Microsoft olvidó subir el prefijo de su proveedor (trac-9572)
...
( función() {
	...
	// Comprobar estado perdido si el nombre está configurado (trac-11217)
...
función getAll(contexto, etiqueta) {
	...
	// Use typeof para evitar la invocación del método de argumento cero en los objetos host (trac-15151)
	varret;
	...
	para ( ; yo < l; i++ ) {
		...
		si ( elemento || elemento === 0 ) {
			...
			// Convertir html en nodos DOM
				...
				// Asegurarse de que los nodos creados sean huérfanos (trac-12392)
				tmp.textContent = "";
			}
		}
	}
	...
	manejadores: función (evento, manejadores) {
		...
		if (cuentadelegados &&
			...
			for ( ; cur !== esto; cur = cur.parentNode || esto ) {
				...
				// No marque elementos que no sean (trac-13208)
			...
			if ( ( event.isTrigger & 1 ) && este[ tipo ] ) {
				...
				if ( !salvado.longitud) {
					...
					if ( guardado !== resultado ) {
						...
						// Soporte: Chrome 86+
						// En Chrome, si un elemento que tiene un controlador de enfoque está borroso por
						// al hacer clic fuera de él, invoca el controlador de forma síncrona. Si
						// ese controlador llama a `.remove()` en el elemento, los datos se borran,
						// dejando `resultado` sin definir. Tenemos que protegernos de esto.
						devolver resultado && resultado.valor;
					}
...
jQuery.each( { foco: "enfocar", desenfoque: "enfocar" }, función (tipo, tipo de delegado) {
	jQuery.event.special[ tipo ] = {
		...
		// Suprimir enfoque nativo o desenfoque si estamos dentro
		// una pila de eventos nativos apalancados
		_predeterminado: función (evento) {
			return dataPriv.get(evento.objetivo, tipo);
		},
...
función domManip (colección, argumentos, devolución de llamada, ignorado) {
	...
	si (l) {
		...
		if (primero || ignorado) {
			...
			// siendo vaciado incorrectamente en ciertas situaciones (trac-8070).
			para ( ; yo < l; i++ ) {
				nodo = fragmento;
				...
				for ( i = 0; i < hasScript; i++ ) {
					...
					if ( rscriptType.test( nodo.tipo || "" ) &&
						...
						if ( nodo.src && ( nodo.tipo || "" ).toLowerCase() !== "módulo" ) {
							...
							// Desempaquetar una sección CDATA que contiene contenido de secuencia de comandos. esto no debería ser
							// necesario porque en los documentos XML ya no son visibles cuando
							// inspeccionando el contenido de los elementos y en los documentos HTML no tienen
							// significado, pero estamos preservando esa lógica para la compatibilidad con versiones anteriores.
...
( función() {
	...
	// Soporte: IE <=9 - 11 solamente
	// El estilo del elemento clonado afecta el elemento fuente clonado (trac-8908)
	...
	jQuery.extender (soporte, {
		...
		dimensionesTr confiables: función () {
			...
			if (valor de las dimensiones confiables == nulo) {
				...
				table.style.cssText = "posición: absoluta; izquierda: -11111px; borde-colapso: separado";
				...
				// Asegurándose de que el div se muestre: bloque
...
function curCSS( elemento, nombre, computado ) {
	...
	si ( calculado ) {
		...
		// solo está disponible como `"cssFloat"`. Ya no modificamos propiedades
		// enviado a `.css()` además de camelCasing, por lo que debemos verificar ambos.
		...
		// elementos desconectados. Sin embargo, en IE incluso elementos desconectados
		// sin estilos devuelve `"ninguno"` para `getPropertyValue( "float" )`
		ret = calculado.getPropertyValue( nombre ) || computado[ nombre ];

		if (esPropiedadPersonalizada && ret) {
			...
			// ambos espacios en blanco iniciales y finales en un solo espacio.
			//
			// Vuelve a `undefined` si se devuelve una cadena vacía.
			// Esto colapsa una definición que falta con la propiedad definida
			// y se establece en una cadena vacía pero no hay una API estándar
			// permitiéndonos diferenciarlos sin una penalización de rendimiento
			// y devolver `undefined` se alinea con jQuery anterior.
			//
			// rtrimCSS trata U+000D RETORNO DE CARRO y U+000C ALIMENTACIÓN DE FORMULARIO
			// como espacio en blanco mientras que CSS no lo hace, pero esto no es un problema
			// porque el preprocesamiento CSS los reemplaza con U+000A LINE FEED
			// (que *es* espacio en blanco CSS)
			// https://www.w3.org/TR/css-syntax-3/#input-preprocesamiento
...
jQuery.extender( {
	...
	estilo: función (elemento, nombre, valor, extra) {
		...
		if ( valor !== indefinido ) {
			...
			// Asegúrese de que los valores nulo y NaN no estén establecidos (trac-7116)
			...
			if ( jQuery.css( elem, "float" ) === "ninguno" ) {
				...
				if ( ! propTween ) {
					...
					if (restoreDisplay == null) {
						...
						restoreDisplay = mostrar === "ninguno" ? "" : mostrar;
					}
				}
	...
	propHooks: {
		índice de tabulación: {
			obtener: función (elemento) {
				...
				// Usar la recuperación de atributos adecuada (trac-12072)
			...
			devuelve esto. cada uno (función () {
				...
				si ( cur ) {
					...
					if (valorActual!== ValorFinal) {
						this.setAttribute( "clase", finalValue );
					}
				}
			});
		}
		...
		opción: {
			obtener: función (elemento) {
				...
				valor devuelto! = nulo?
					...
					// option.text arroja excepciones (trac-14686, trac-14858)
		...
		Seleccione: {
			obtener: función (elemento) {
				...
				para ( ; i < max; i++ ) {
					...
					// Soporte: IE <=9 solamente
					// IE8-9 no actualiza la selección después de restablecer el formulario (trac-2551)
...
jQuery.extend( jQuery.evento, {
	...
	disparador: función (evento, datos, elemento, solo manejadores) {
		...
		// Determinar la ruta de propagación de eventos por adelantado, según la especificación de eventos W3C (trac-9951)
		// Burbuja hasta documento, luego a ventana; busque un propietario globalDocumento var (trac-9724)
	...
	jQuery.each( { foco: "focusin", desenfoque: "focusout" }, function( orig, fix ) {
		...
		jQuery.event.special[ arreglar ] = {
			configuración: función () {
				...
				var doc = este.documentopropietario || este.documento || esto,
...
variable
	...
	// Evite la secuencia de caracteres comment-prolog (trac-10098); debe apaciguar la pelusa y evadir la compresión
...
jQuery.extender( {
	...
	ajax: función (url, opciones) {
		...
		// Manejar la URL falsa en el objeto de configuración (trac-10093: coherencia con la firma anterior)
		// También usamos el parámetro url si está disponible
		s.url = ( ( url || s.url || ubicación.href ) + "" )
			.replace( rprotocol, ubicación.protocolo + "//" );

		// Opción de método de alias para escribir según ticket trac-12004
		...
		// No activar eventos si jQuery.event no está definido en un escenario de uso de AMD (trac-15118)
		...
		if ( !s.hasContent ) {
			...
			if ( s.data && ( s.processData || typeof s.data === "string") ) {
				...
				// trac-9682: eliminar datos para que no se utilicen en un eventual reintento
				eliminar s.datos;
			}
		...
		función hecha (estado, nativeStatusText, respuestas, encabezados) {
			...
			// Use un convertidor de noop para el script que falta, pero no si es jsonp
			if ( !esÉxito &&
				jQuery.inArray( "secuencia de comandos", s.tipos de datos) > -1 &&
				jQuery.inArray( "json", s.tipos de datos ) < 0 ) {
...
jQuery._evalUrl = función (url, opciones, documento) {
	devuelve jQuery.ajax( {
		...
		// Hacer esto explícito, ya que el usuario puede anularlo a través de ajaxSetup (trac-11264)
		tipo: "OBTENER",
							...
							if ( tipo === "abortar" ) {
								...
								if ( tipo de xhr.status !== "número" ) {
									...
									completo(
										...
										// Archivo: el protocolo siempre arroja el estado 0; ver trac-8605, trac-14207
										xhr.estado,
										xhr.statusText
									);
								}
...
jQuery.fn.extender( {
	...
	posición: función() {
		...
		if ( jQuery.css( elem, "posición" ) === "fijo" ) {
			...
			// Asumir position:fixed implica disponibilidad de getBoundingClien tRect
...
jQuery.each ({ Altura: "altura", Ancho: "ancho"}, función (nombre, tipo) {
	jQuery.each( {
		...
		tipo de contenido,
		"": "exterior" + nombre
	}, función (predeterminado Extra, nombre de función) {
...
jQuery.each(
	( "difuminar enfocar enfocar dentro enfocar fuera cambiar el tamaño desplazarse hacer clic doble clic " +
...
// para evitar el comportamiento O(N^2) cuando el motor intenta hacer coincidir "\s+$" en cada posición de espacio.
